...
void jitc_var_cond_end(uint32_t index, uint32_t *rv_out) {
    ...
    size_t side_effects = 0;

    jitc_log(InfoSym,
             "jit_var_cond(cond=r%u): created a conditional statement (\"%s\") "
             "with %zu/%zu state variable%s (%zu/%zu bytes), %zu side effect%s, "
             "array size %u.", index, cd->name.c_str(), n_vars_actual,
             cd->indices_t.size(), n_vars_actual== 1 ? "": "s",
             storage, storage_actual, side_effects, side_effects == 1 ? "": "s", 0);
}
